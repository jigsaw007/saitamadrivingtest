...
const SetA = () => {
  ...
  useEffect(() => {
    fetch(`${process.env.PUBLIC_URL}/data/setA.json`)
  ...
  const saveScore = (score, totalQuestions) => {
    ...
    savedScores.setA.unshift(scoreEntry); // Add new score to the beginning of the array
    ...
    <div>
      ...
      <div className="container mt-5 p-5 rounded shadow" style={{ backgroundColor: "#ffffff" }}>
        ...
        <p className="text-center mb-4" style={{ fontSize: "18px", fontFamily: "'Roboto', sans-serif" }}>
          Test your knowledge with the Set A Quiz. Answer all questions and aim for the highest score!
        </p>
        {!isPremiumUser() && (
          <Paywall title="Unlock full Set A quiz" />
        )}
